...
*   **Story: As a user who forgot my password, I want to reset it securely.**
    *   [x] **Database:** Add columns to the `users` table: `password_reset_token` (VARCHAR) and `password_reset_expires` (TIMESTAMP).
    *   [x] **UI/UX:** Design a "Forgot Password" page that takes a user's email address.
    *   [x] **Backend API:** Create an endpoint `POST /api/v1/auth/forgot-password`.
    *   [x] **Backend Logic:** This endpoint should generate a unique, cryptographic, time-sensitive reset token, store its hash and expiration in the database for the user, and email the user a reset link containing the token.
    *   [x] **UI/UX:** Design a "Reset Password" page with "New Password" and "Confirm New Password" fields. This page is accessed via the link from the email.
    *   [x] **Backend API:** Create an endpoint `POST /api/v1/auth/reset-password/:token`.
    *   [x] **Backend Logic:** This endpoint will validate the token from the URL, check that it has not expired, update the user's `password_hash` with the new hashed password, and then invalidate the reset token.
    *   [x] **Testing:** Write unit tests for both the `forgot-password` and `reset-password` flows.
...
*   **Story: As a seller, I want to create a detailed costume listing.**
    *   [x] **Database:** Create a `costumes` table (`id`, `seller_id` (FK to users), `title`, `description`, `price` (DECIMAL), `size` (VARCHAR), `condition` (ENUM: 'New', 'Like New', 'Good', 'Fair'), `status` (ENUM: 'available', 'sold'), `created_at`, `updated_at`).
    *   [x] **Database:** Create a `costume_categories` table (`id`, `name`). Pre-populate with styles: Ballet, Jazz, Tap, Lyrical, Contemporary, Hip-Hop, etc.
    *   [x] **Database:** Create a `costume_to_category` join table (`costume_id`, `category_id`).
    *   [x] **Database:** Create a `costume_images` table (`id`, `costume_id` (FK to costumes), `image_url`, `is_primary` (BOOLEAN)).
    *   [x] **UI/UX:** Design a multi-step "Create Listing" form.
    *   [x] **Frontend (Step 1):** Implement form fields for Title (text input), Description (textarea), and Categories (multi-select checkboxes).
    *   [x] **Frontend (Step 2):** Implement form fields for Price (numeric input), Size (dropdown: Child S/M/L, Adult XS/S/M/L/XL), and Condition (radio buttons).
    *   [x] **Frontend (Step 3):** Implement a multi-file uploader with drag-and-drop support, image previews, and the ability to reorder images and select one as the primary display image.
    *   [x] **Frontend (Step 4):** Implement form fields for shipping options and costs.
    *   [x] **Frontend:** Implement comprehensive client-side validation for all fields.
    *   [x] **Backend API:** Create an endpoint `POST /api/v1/costumes` to handle the creation of a new listing.
    *   [x] **Backend Logic:** The endpoint must handle the upload of multiple images to the cloud storage bucket.
    *   [x] **Backend Logic:** The endpoint must create the main `costume` record and then create associated records in `costume_images` and `costume_to_category`.
    *   [x] **Frontend Logic:** On successful submission, redirect the seller to the newly created listing's public page.
    *   [x] **Testing:** Write unit tests for the listing creation endpoint, including image handling.

*   **Story: As a seller, I want to manage my active and sold listings.**
    *   [x] **UI/UX:** Design a "My Listings" dashboard page with tabs for "Active" and "Sold".
    *   [x] **UI/UX:** Each listing in the dashboard view should show its primary image, title, price, and current status.
    *   [x] **Backend API:** Create an endpoint `GET /api/v1/users/me/costumes` to fetch all listings for the currently authenticated seller.
    *   [x] **Frontend:** Implement an "Edit" button for each active listing that navigates to a pre-filled version of the "Create Listing" form.
    *   [x] **Backend API:** Create an endpoint `GET /api/v1/costumes/:id` to fetch data for a single costume.
    *   [x] **Backend API:** Create an endpoint `PUT /api/v1/costumes/:id` to handle updates to an existing listing. Ensure a user can only edit their own listings.
    *   [x] **Frontend:** Implement a "Delete" button for active listings, which triggers a confirmation modal before proceeding.
    *   [x] **Backend API:** Create an endpoint `DELETE /api/v1/costumes/:id`. Ensure a user can only delete their own listings.
    *   [x] **Backend Logic:** Ensure that a listing cannot be edited or deleted once it has been sold.
...
*   **Story: As a buyer, I want to easily find costumes using search and filters.**
    *   [x] **UI/UX:** Design the homepage to feature a prominent search bar and a grid of costume categories.
    *   [x] **UI/UX:** Design the search results page with a grid/list view of costumes and a sidebar for filtering options.
    *   [x] **Backend API:** Create a search endpoint `GET /api/v1/costumes/search` that supports a query parameter for full-text search on `title` and `description`.
    *   [x] **Frontend:** Implement the filter sidebar UI with controls for: Category (checkboxes), Size (checkboxes), Price Range (a dual-handle slider), and Condition (checkboxes).
    *   [x] **Backend Logic:** Enhance the search endpoint to accept and apply filter parameters to the database query.
    *   [x] **Backend Logic:** Implement pagination on the search endpoint to handle large result sets efficiently (e.g., returning 24 items per page).
    *   [x] **Frontend Logic:** Update the search results dynamically as filters are applied, without requiring a full page reload.
...
*   **Story: As a buyer, I want to use a shopping cart and complete a secure purchase.**
    *   [x] **UI/UX:** Implement a shopping cart icon in the site header that updates with the number of items.
    *   [x] **Frontend:** Implement client-side state management for the shopping cart.
    *   [x] **UI/UX:** Design the shopping cart page, showing a list of items with options to remove them. Display subtotal, shipping, and total.
    *   [x] **UI/UX:** Design a multi-step or single-page checkout flow that collects a shipping address.
    *   [x] **Database:** Create an `orders` table (`id`, `buyer_id`, `total_price`, `status`, `shipping_address`, `created_at`).
    *   [x] **Database:** Create an `order_items` table (`id`, `order_id`, `costume_id`, `price_at_purchase`).
    ...
    *   [x] **Backend API:** Create an endpoint `POST /api/v1/checkout` to process the order.
    *   [x] **Backend Logic:** The checkout endpoint must:
...
*   **Story: As a buyer, I want to view my order history.**
    *   [x] **UI/UX:** Design a "My Orders" page for buyers to see a list of past purchases.
    *   [x] **Backend API:** Create an endpoint `GET /api/v1/users/me/orders` to fetch the logged-in user's order history.
    *   [x] **Frontend:** Display key information for each order: order date, order total, and status (e.g., 'Processing', 'Shipped').
